...
data "template_file" "vault_config" {
  ...
  vars = {
    ...
    api_addr           = "https://${format("10.10.42.22%d", count.index)}:8200"
    cluster_addr       = "https://${format("10.10.42.22%d", count.index)}:8201"
    cluster_address    = "${format("10.10.42.22%d", count.index)}:8201"
...
data "template_file" "vault_oss_storage_config" {
  ...
  vars = {
    ...
    disable_clustering = var.disable_clustering
    service_tags       = "vaultron"
    node_id            = "vaultrondr-vault-${count.index}"
  }
}
...
resource "docker_container" "vault_oss_server" {
  count = var.vault_oss_instance_count
  name  = "vaultrondr-${format("vault%d", count.index)}"
  image = docker_image.vault.latest

  env = ["SKIP_CHOWN", "VAULT_CLUSTER_ADDR=https://${format("10.10.42.22%d", count.index)}:8201", "VAULT_REDIRECT_ADDR=https://${format("10.10.42.22%d", count.index)}:8200", "VAULT_LOG_FORMAT=${var.vault_server_log_format}"]
  ...
  networks_advanced {
    ...
    ipv4_address = format("10.10.42.22%d", count.index)
  }
  ...
  ports {
    internal = "8200"
    external = format("92%d0", count.index)
    protocol = "tcp"
  }
}
...
resource "docker_container" "vault_custom_serverdr" {
  count    = var.vault_custom_instance_count
  name     = "vaultrondr-${format("vault%d", count.index)}"
  image    = docker_image.vault.latest
  env      = ["SKIP_CHOWN", "VAULT_CLUSTER_ADDR=https://${format("10.10.42.22%d", count.index)}:8201", "VAULT_REDIRECT_ADDR=https://${format("10.10.42.22%d", count.index)}:8200", "VAULT_LOG_FORMAT=${var.vault_server_log_format}", "VAULT_LICENSE=${var.vault_license}"]
